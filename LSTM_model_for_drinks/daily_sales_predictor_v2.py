# V2 Model looking into 'Date', 'Total_Items_Sold', 'Day_of_Week', 'Is_Weekend', 'Is_Peak_Hour'
...
epsilon = 1e-3
mask = y_test_inv.flatten() > epsilon
mape = np.mean(np.abs((y_test_inv.flatten()[mask] - y_pred_inv.flatten()[mask]) / y_test_inv.flatten()[mask])) * 100 if np.any(mask) else np.nan
...
plt.title(f'LSTM Forecast (LR={learning_rate}): Items Sold (RMSE: {rmse:.2f}, MAE: {mae:.2f}, MAPE: {mape:.2f}%)')
plt.xlabel('Time Step')
plt.ylabel('Items Sold')
plt.legend()
plt.grid()
plt.savefig('../images/Combined_LSTM_Forecast_V2.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig('../images/Error_Distribution_and_Actual_vs_Predicted_V2.png', dpi=300, bbox_inches='tight')
plt.show()
# %%
